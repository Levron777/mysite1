...
/*Старый mysqli код
		$total_articles = mysqli_query($mysqli, "SELECT COUNT(`id`) AS `total_count` FROM `articles` WHERE `category` = 'Russia'");
                $total_count = mysqli_fetch_assoc($total_articles);*/
        $total_articles = $pdo->query("SELECT COUNT(`id`) AS `total_count` FROM `articles`WHERE `category` = 'Russia'");
		...
		/*$articles = mysqli_query($mysqli, "SELECT * FROM `articles` WHERE `category` = 'Russia' ORDER BY `id` DESC LIMIT $art, $per_page");*/
        $articles = $pdo->prepare("SELECT * FROM `articles` WHERE `category` = 'Russia' ORDER BY `id` DESC LIMIT ?, ?");
        ...
        if ( $articles_exist == true ) {
            ...
            if( $page > 1 ) {
                echo '<a class="prev_page" href="/pages/russia.php?page='.($page - 1).'"> ← Предыдущая страница | </a>';
            }
                if( $page < $total_pages ) {
                    echo '<a class="next_page" href="/pages/russia.php?page='.($page + 1).'">Следующая страница  → </a>';
                }
            echo '</div>';
        }
