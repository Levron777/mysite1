...
/*Старый mysqli код
		$total_articles = mysqli_query($mysqli, "SELECT COUNT(`id`) AS `total_count` FROM `articles`");
		...
		//$articles = mysqli_query($mysqli, "SELECT * FROM `articles` ORDER BY `id` DESC LIMIT $art, $per_page");
        ...
        if( $page > 1 ){
            echo '<a class="prev_page" href="/index.php?page='.($page - 1).'"> ← Предыдущая страница |  </a>';
        }

        if( $page < $total_pages ) {
            echo '<a class="next_page" href="/index.php?page='.($page + 1).'">Следующая страница  → </a>';
        }
            echo '</div>';
        }
